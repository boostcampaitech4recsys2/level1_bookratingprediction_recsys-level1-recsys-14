...
from src import HybridNeuralCollaborativeFiltering
...
def main(args):
    ...
    elif args.MODEL in ('NCF', 'WDN', 'DCN', 'HYBRID'):
    ...
    ### idx2user, idx2isbn
    idx2user, idx2isbn = data['idx2user'], data['idx2isbn']
    ...
    if args.MODEL=='FM':
        ...
        model = HybridNeuralCollaborativeFiltering(args, data)
    else:
        pass
    ...
    # ######################## cold start
    # cold_start_user = pd.merge(data['train'], data['test'], how='outer', indicator=True).query('_merge == "right_only"').drop(columns=['_merge'])
    idx2user = data
...
if __name__ == "__main__":
    ...
    arg('--MODEL', type=str, choices=['FM', 'FFM', 'NCF', 'WDN', 'DCN', 'CNN_FM', 'DeepCoNN', 'HYBRID'],
