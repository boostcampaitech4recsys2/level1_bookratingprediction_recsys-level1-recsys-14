...
def main(args):
    ...
    if args.MODEL in ('FM', 'FFM'):
        ...
        import nltk
        nltk.download('punkt')
        data, r_avg, idx2isbn = text_data_load(args)
    else:
        pass
    ...
    if args.MODEL in ('FM', 'FFM'):
        ...
        data, train_user = text_data_split(args, data)
    ...
    # minimum_loss = model.train(train_user, r_avg) # csv 파일에 minimum loss 넣어서 저장
    minimum_loss = model.train()
    ...
    if args.MODEL in ('FM', 'FFM', 'NCF', 'WDN', 'DCN'):
        ...
        predicts  = model.predict(data['test_dataloader'], train_user, r_avg, idx2isbn)
    else:
        pass
    ...
    submission.to_csv('submit/{}_{}.csv'.format('_'.join(args for args in sys.argv[2::2]), minimum_loss), index=False)
